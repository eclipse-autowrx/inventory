...
setInterval(() => {
  const now = Date.now();
  const entries = throttleLoggers.entries();
  for (let i = 0; i < entries.length; i += 1) {
    const [key, concreteLogger] = entries[i];
    const timeSyncLastTouched = now - concreteLogger.lastTouched;
    if (!concreteLogger.timer && timeSyncLastTouched >= STALE_THRESHOLD) {
      concreteLogger
        ._logDataUpdate()
        .catch((error) => {
          concreteLogger.error(`Error logging stale data for ${key}: %o`, error?.message || error);
        })
        .finally(() => {
          throttleLoggers.delete(key);
        });
